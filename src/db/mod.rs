...
pub async fn get_connection_status(pool: &PgPool) -> Result<DbConnectionStatus, sqlx::Error> {
    let (database, host, port, user) =
        sqlx::query_as::<_, (String, Option<String>, Option<i32>, String)>(
            r#"
        SELECT
            current_database(),
...
pub async fn insert_rss_item(pool: &PgPool, item: &RssItem) -> Result<bool, sqlx::Error> {
    ...
    .bind(item.id) // UUID generated by source + title + published_at
    .bind(&item.source)
    .bind(&item.title)
    .bind(&item.link)
    .bind(&item.summary)
